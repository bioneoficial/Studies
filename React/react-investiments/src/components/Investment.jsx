...
export default function Investment({ description, children: reports }) {
  ...
  const lastReport = parseFloat(
    newOrdenedReports[newOrdenedReports.length - 1].value
  );
  ...
  const totalRentBgColor =
    totalRent > 0 ? 'text-green-500' : totalRent < 0 ? 'text-red-500' : '';
  return (
    <div className=" border rounded-lg border-black w-4/5 sm:w-4/5 md:w-4/5 lg:w-3/5 mx-auto my-3 p-2 ">
      <h1 className="text-lg font-semibold text-center">{description}</h1>
      <p className="text-sm text-center mb-5 font-semibold">
        Total Income:{' '}
        <span className={totalRentBgColor}>
          R$ {totalRent} ({sTotalPercent}%)
        </span>
      </p>
